...
public class PageInfo { 
	...
	private int listCount; // 현재 일반게시판의 총 게시글 수 => BOARD로부터 조회 COUNT(*)활용(STATUS='Y')
	private int currentPage; // 현재 페이지(사용자가 요청한 페이지) => request.getParameter("cpage");
	private int pageLimit; // 페이지 하단에 보여질 페이징 바의 최대 개수(버튼 몇개보여줄거냐) => 10개로 고정
	private int boardLimit; // 한 페이지에 보여질 게시글의 최대 수 => 10개로 고정
