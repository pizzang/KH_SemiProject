...
public class PageInfo {
	...
	private int listCount; // 현재 일반게시판의 게시글 총 갯수 => BOARD로 부터 COUNT(*)활용(STATUS = 'Y')
	private int currentPage; // 현재 페이지(즉, 사용자가 요청한 페이지) => request.getParameter("cpage");
	private int pageLimit; // 페이지 하단에 보여질 때 페이징바의 최대 갯수 => 10개로 고정
	private int boardLimit; // 한 페이지에 보여질 게시글의 최대 갯수 => 10개로 고정
	
	private int maxPage; // 가장 마지막 페이지가 몇번 페이지 인지(총 페이지 갯수)
